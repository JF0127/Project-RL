...
当前项目仅在 Ubuntu 22.04、ROS2 Humble，以及 Unitree G1 的 Jetson Orin NX 29 自由度版本上进行过测试。
...
按照 `crc_module` 的说明进行安装，并将产物文件 `crc_module.so` 复制到你运行 Python 脚本的位置。
...
- 安装板载侧 Python 包，并自动检测 GPU，但默认不包含 OpenCV 相关库：

```bash
pip install -e .
```

这会自动检测当前环境中是否有可用的 GPU 或 CUDA，并安装对应版本的 ONNX Runtime。
