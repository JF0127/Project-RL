...
parser.add_argument("--wandb_project", type=str, default=None, help="WandB project name to enable logging.")
parser.add_argument("--wandb_entity", type=str, default=None, help="WandB entity (username or team name).")
parser.add_argument("--wandb_run_name", type=str, default=None, help="Specific name for the WandB run.")
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    # ========================== 新增代码开始 ==========================
    # 只有当传入了 --wandb_project 参数，且处于主进程 (Rank 0) 时才初始化 wandb
    enable_wandb = args_cli.wandb_project is not None
    if args_cli.distributed:
        # 在分布式模式下，检查 local_rank 是否为 0
    ...
    if enable_wandb:
        run_name = (
            ...
            else f"{agent_cfg.experiment_name}_{datetime.now().strftime('%m%d_%H%M')}"
        )
        ...
        wandb.init(
            ...
            name=run_name,
            # 将环境和代理的配置上传到 wandb 方便溯源
            config={"env_cfg": env_cfg.to_dict(), "agent_cfg": agent_cfg.to_dict()},
            # 关键：同步 TensorBoard 日志
            # rsl-rl 默认写入 TensorBoard，这行代码会自动抓取并上传到 wandb
            sync_tensorboard=True,
            dir=log_dir,  # 将 wandb 的本地元数据也存在实验目录里
        )
        print(f"[INFO] WandB initialized. Run name: {run_name}")
    # ========================== 新增代码结束 ==========================
