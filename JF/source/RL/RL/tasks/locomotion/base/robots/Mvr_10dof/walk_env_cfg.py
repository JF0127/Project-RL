...
from RL.assets.robots.mvr import MVR_10DOF_CONFIG
...
class RobotSceneCfg(InteractiveSceneCfg):
    ...
    # robot
    robot: ArticulationCfg = MVR_10DOF_CONFIG.replace(prim_path="{ENV_REGEX_NS}/Robot")
...
class ActionsCfg:
    ...
    JointPositionAction = mdp.JointPositionActionCfg(
        asset_name="robot",
        joint_names=DESIRED_JOINT_ORDER,
        scale=0.25,
...
class RewardsCfg:
    ...
    track_lin_vel_xy = RewTerm(
        ...
        weight=8.0,  # 提高：让速度跟踪优先于 gait，减少原地踱步
    ...
    joint_vel = RewTerm(func=mdp.joint_vel_l2, weight=-0.005)  # 加强：抑制腿部高频抖动
    joint_acc = RewTerm(func=mdp.joint_acc_l2, weight=-5e-7)   # 加强：抑制关节加速度冲击
    action_rate = RewTerm(func=mdp.action_rate_l2, weight=-0.1)  # 加强：减少动作高频切换
    dof_pos_limits = RewTerm(func=mdp.joint_pos_limits, weight=-5.0)
    energy = RewTerm(func=mdp.energy, weight=-5e-5)  # 加强：惩罚无效能耗
    ...
    base_height = RewTerm(func=mdp.base_height_l2, weight=-10, params={"target_height": 0.54})

    # -- feet (MVR feet = ankle_pitch_link)
    gait = RewTerm(
        func=mdp.feet_gait,
        weight=5.0,  # 降低：减弱"不论是否前进都踱步"的激励
        params={
            "period": 0.8,  # 缩短步态周期，鼓励更紧凑、干净的步伐而非拖沓踱步
            "offset": [0.0, 0.5],
            "threshold": 0.55,
    ...
    feet_slide = RewTerm(
        func=mdp.feet_slide,
        weight=-8.0,  # 加强：更严格惩罚脚部滑动（踱步常伴随滑动）
        params={
    ...
    feet_flat = RewTerm(
        ...
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names=["left_ankle_pitch_link", "right_ankle_pitch_link"]),
        },
    )
